...
(() => {
     ...
     * list of tasks
     */
    document.onreadystatechange = () => Ajax.sendGetRequest(API_URL, {}, MediaFormat.JSON, loadTasks, error => showError(error), true);
    ...
    const addTask = (e) => {
        ...
        e.preventDefault();
        Ajax.sendPostRequest(API_URL, {description: content}, MediaFormat.JSON, data => {
    ...
    const addOnChangeEvent = (task) => {
        ...
        checkBox.onchange = (e) => {
            ...
            Ajax.sendPutRequest(`${API_URL}/${task.id}`, {status: newState ? 'TERMINADO' : 'PENDIENTE'}, MediaFormat.JSON, data => {
                document.getElementById(`task-${task.id}`).remove();
                addTaskToList(JSON.parse(data));
            }, code => showError(code, 'Error al actualizar registro'), true);
    ...
    const editTask = (e) => {
        ...
        buttonOK.onclick = () => {
            ...
            Ajax.sendPutRequest(`${API_URL}/${currentTask.id}`, {description: currentTask.description}, MediaFormat.JSON, (data) => revertHTMLChangeOnEdit(data), error => showError(error), true)
        };
